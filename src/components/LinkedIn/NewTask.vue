    ...
    <div id="newtask">
        ...
        <el-card class="box-card">
            <el-form :model="dynamicValidateForm2" ref="dynamicValidateForm2" label-width="100px" class="demo-dynamic">
                <el-form-item
                    ...
                    <el-col :span="1">&nbsp;</el-col>
                    <el-button @click.prevent="removeDomain1(domain)" type="danger" icon="el-icon-delete" circle></el-button>
                </el-form-item>
                <el-form-item style="margin-bottom:0px">
                    <div class="button2">
                        <el-button class="ripple" @click="addDomain1">ADD ID</el-button>
                    </div>
                </el-form-item>
            </el-form>
            <!-- 文件上传 -->
                ...
                <el-form-item label="FileUpload:" prop="upload" label-width="90px" style="padding-left:15px;">
                    ...
                    <el-col :span="20">
                        <el-upload
                            ...
                            multiple
                            :auto-upload="false"
                            :on-change="loadJsonFromFile1"
                            :limit="1"
                            :on-exceed="handleExceed"
                            :on-remove="handleRemove1"
                            :file-list="uploadFiles1"
                            accept="text/plain">
                        <i class="el-icon-upload"></i>
                        <div class="el-upload__text">Drag files here, or <em>click Upload</em></div>
                        <div class="el-upload__tip" slot="tip">（Only supports .txt files.The content format is["Name1","Name2",...]）</div>
        ...
        <el-card class="box-card">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                <el-form-item label="Settings:" prop="config" style="margin-left:-10px;">
                    <el-form-item label="HopCount:" prop="name1" style="padding-bottom:15px">
                        <el-input v-model="ruleForm.name1" placeholder="0-10" style="margin-left:24px;width:100px;"></el-input>
                    </el-form-item>
                    <el-form-item label="CrawlRate:" prop="name2" style="padding-bottom:15px" hidden>
                        <el-radio-group v-model="radio">
                            <el-radio style="padding-right:30px">
                                <el-input v-model="ruleForm.name2" placeholder="0:unlimited" style="width:100px"></el-input> times/minute
                            </el-radio>  
            ...
            <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                <el-form-item label="CrawlContent:" prop="contentconfig" style="padding-left:15px;">
                    ...
                    <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">   
                        <div class="card1" v-for="(item,index) in this.commentData" :key="index" >
                            <el-col :span="17">
                                <el-checkbox 
                                    :label="item.name" 
                                    :id="item.name" 
...
<script>
const cityOptions = ['UserInfo','Friends', 'Company'];
export default {
    name: 'NewTask',
    inject:['reload'],
    data() {
        ...
        var targetidVerify = (rule, value, callback) => {
            ...
            {
                if (!value) {
                    return callback(new Error('UserID cannot be empty'));
                }
                else
                    callback();
            }
        ...
        var companyidVerify = (rule, value, callback) => {
            if(this.uploadFiles1.length==0)
            {
                if (!value) {
                    return callback(new Error('CompanyID cannot be empty'));
                }
                else
                    callback();
            }
        ...
        return {
            ...
            // selectionData: [],
            checkList:[],
            checkedCities: ['UserInfo','Friends', 'Company'],
            cities: cityOptions,
            ...
            rules3: {
                validator: companyidVerify, trigger: 'blur'
            },
            // 主机选择
            value1: [0],
            ...
            dynamicValidateFormValid2: false,
            commentData: [
                { name: 'UserInfo', des: [0], msg:'Basic information: user name, account name, marriage, date of joining. \n     Introduction: work and education, places where people live, contact information and basic conditions, family members, detailed information, life rules.'},
                { name: 'Friends', des: [0], msg: 'User\'s friends information, including friend\'s name, account number ,location and avatar, etc.'},
                { name: 'Company', des: [0], msg: 'User\'s company information, including company\'s description, staff account, found time, locations, etc.'}
            ]
        }
    },
    mounted () {
    ...
    methods: {
        ...
        //读取companyID文件内容，目前仅支持读取一个文件
        loadJsonFromFile1 (file, fileList) {
            this.uploadFilename1 = file.name
            this.uploadFiles1 = fileList
            if (this.uploadFiles1) {
                let file = this.uploadFiles1[0]
                let reader = new FileReader()
                reader.onload = async (e) => {
                    try {
                        let document = JSON.parse(e.target.result)
                        console.log(document)
                        this.Filescontent1 = document
                        // console.log(this.Filescontent)
                    } catch (err) {
                        this.$message({
                        type: 'error',
        ...
        selectGet2(vId,index){
            if(index === "UserInfo")
            {
                this.obj1 = []
                for(let i in vId)
                    this.obj1[i] = this.options1[vId[i]]
            }
            if(index === "Friends")
            {
                this.obj2 = []
        ...
        submitAllForm() {
            ...
            if(this.options.length==0)
            {
                ...
                .catch(action => {
                    this.$router.push({path:'/LinkedIn/HostConfig'})
                });
            }
            //先验证合法性
            ...
            this.$refs.dynamicValidateForm2.validate((valid) => {
                if (valid) {
                    this.dynamicValidateFormValid2 = true
                    console.log("3")
                }
            })
            ...
            if(this.ruleForm2Valid && this.dynamicValidateFormValid && this.dynamicValidateFormValid2 && this.ruleFormValid) {
                var namedata = this.ruleForm2.taskname
                var startUsersdata = []
                var startCompaniesdata = []
                var configdata = {}
                var hostMapdata = {}
                var hopcount
                var rate
            ...
            //startuser
                ...
                for(var i=0;i<this.dynamicValidateForm2.domains.length;i++)
                    startCompaniesdata.push(this.dynamicValidateForm2.domains[i].value)
                    // console.log(this.Filescontent)
                    //若未输入账号，则读取文件中
                    let l=0
                    for(let i=0;i<startCompaniesdata.length;i++)
                        if(startCompaniesdata[i]==='')
                            l++
                    if(l==startCompaniesdata.length)
                        startCompaniesdata = []
                    //如果文件中含有账号，则读取输入加文件
                    if(this.Filescontent1!='')
                        startCompaniesdata = startCompaniesdata.concat(this.Filescontent1)
                    // console.log(startCompaniesdata)
                var FilescontentData2=[]
                FilescontentData2 = this.Filescontent1
                //判断是否删除了该文件,若是，则从数组里删除
                if(this.removed1==true)
                {
                    for(let i=0;i<FilescontentData2.length;i++){
                        for(let j=0;j<startCompaniesdata.length;j++){
                            if(startCompaniesdata[j]== FilescontentData2[i]){
                                startCompaniesdata.splice(j, 1);
                                j--;
                            }
                        }
                    }
                ...
                configdata.friends_hop = hopcount
                // console.log(configdata)
                for(var i=0;i<this.checkedCities.length;i++){
                    if(this.checkedCities[i]==="UserInfo")
                    {
                        if(this.obj1.length === 0)
                            this.obj1 = this.options1[0]
                        hostMapdata.userinfo = this.obj1
                    }
                    if(this.checkedCities[i]==="Friends")
                    {
                        if(this.obj2.length === 0)
                            this.obj2 = this.options1[0]
                        hostMapdata.friends = this.obj2
                    }
                    if(this.checkedCities[i]==="Company")
                    {
                        if(this.obj3.length === 0)
                            this.obj3 = this.options1[0]
                        hostMapdata.company = this.obj3
                    }
                }
                //周期性任务和自启性任务数据（新加）
                var periodtype
                ...
                var Params = {
                    ...
                    startCompanies : startCompaniesdata,
                    config : configdata,
                    hostMap : hostMapdata,
                    type : periodtype,
                ...
                var number=0;
                var spider=[];
                for(var item in hostMapdata)
                {
                    number++;
                    spider.push(item)
                }
                if((spider.indexOf("friends")!=-1)&&(number==1))
                {
                    newtask.scrollIntoView();
                    this.$message({
                        type: 'error',
                        message: 'Sorry! You can\'t just choose Friends crawlers.' 
                    });
                }
                //不可不选择爬虫
                else if(number==0)
                {
                    ...
                    this.$message({
                        type: 'error',
                        message: 'Sorry! You have not chosen any crawlers.' 
                    });
                }
                else {
                    //当选择userinfo爬虫时，需填写startUserdata;当选择company爬虫时，需填写startCompaniesdata.
                    if(((spider.indexOf("userinfo")!=-1)&&(startUsersdata.length==0))||((spider.indexOf("company")!=-1)&&(startCompaniesdata.length==0)))
                    {
                        newtask.scrollIntoView();
                        this.$message({
                            type: 'error',
                            message: 'Sorry! Please check whether the UserID or CompanyID is filled in.' 
                        });
                    }
                    else
                    {
                        this.$ajax({
                            url: '/three/linkedin/job/create',
                            method: 'post',
                            contentType: "application/json; charset=utf-8",
                            data: Params
                        }).then( res => {
        ...
        WheatherToWorkStatus() {
            ...
            .catch(action => {
                this.$router.push({path:'/LinkedIn/WorkStation'})
            });
        },
        WheatherToPeriodWorkStatus() {
            ...
            .catch(action => {
                this.$router.push({path:'/LinkedIn/PeriodWorkStation'})
            });
        }
    }
}
