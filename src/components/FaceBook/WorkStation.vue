...
export default {
        ...
        this.$ajax({
            ...
            else
            {
                ...
                for(var p in jsonObj){
                    ...
                    if( p == "content")
                    {
                        if(res1.data.data.content == "running")
                            obj2.status = "running"
                        if(res1.data.data.content == "stopped")
                            obj2.status = "pause"
                    }
                    if( p == "comments")
                    {
                        if(res1.data.data.comments == "running")
                            obj2.status = "running"
                        if(res1.data.data.comments == "stopped")
                            obj2.status = "pause"
                    }
                    if( p == "like")
                    {
                        if(res1.data.data.like == "running")
                            obj2.status = "running"
                        if(res1.data.data.like == "stopped")
                            obj2.status = "pause"
                    }
    ...
    downloadLogs(row){
        this.$confirm('Are you sure to download logs of this task?','NOTICE',{
            ...
            this.$ajax({
                ...
                if(row.spidername === "content")
                    window.location.href=res.data.data.content;
                if(row.spidername === "comments")
                    window.location.href=res.data.data.comments;
                if(row.spidername === "like")
                    window.location.href=res.data.data.like;
                //下载成功做的动作   
                this.$message({
                    type: 'success',
    ...
    //路由跳转方法
    toNewTask: function() {
        this.$router.push({path:'/facebook/NewTask'})
    },
    //计算时间差方法
    timeDifference(date)
    {
