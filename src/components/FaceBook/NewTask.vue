            ...
            <el-form label-width="100px" style="padding-top:30px;">
                ...
                <el-form-item label="FileUpload:" prop="upload" label-width="90px" style="padding-left:15px;">
                    ...
                    <el-col :span="20">
                        ...
                        <div class="el-upload__text">Drag files here, or <em>click Upload dds</em></div>
        ...
        <el-card class="box-card">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                <el-form-item label="Settings:" style="margin-left:-10px;">
                    ...
                    <el-form-item prop="Period" label="Period：" style="padding-left:340px;" class="spical_elform">
...
<script>
const cityOptions = ['Friend', 'TimeLine', 'Like','Comment'];
export default {
    name: 'NewTask',
    inject:['reload'],
    data() {
        ...
        return {
            ...
            // selectionData: [],
            checkList:[],
            checkedCities: ['Friend', 'TimeLine', 'Like','Comment'],
            cities: cityOptions,
            ...
            commentData:[
                { name: 'Friend', des: [0], msg: 'Friends name, friend account, friend hop count.'},
                { name: 'TimeLine', des: [0], msg: 'Posting account number, post ID, content, time of publication, whether it is a repost, or the name of the repost.'},
                { name: 'Like', des: [0], msg: 'Like people\'s names, like people\'s heads, and like people\'s accounts.'},
                { name: 'Comment', des: [0], msg: 'Commentator\'s name, comment content, time, whether to reply.'}
            ]
        }
    },
    mounted () {
    ...
    methods: {
        ...
        selectGet2(vId,index){
            ...
            if(index === "Like")
            {
                this.obj4 = []
                for(let i in vId)
                    this.obj4[i] = this.options1[vId[i]]
            }
            if(index === "Comment")
            {
                this.obj5 = []
                for(let i in vId)
                    this.obj5[i] = this.options1[vId[i]]
            }
        },
        //创建任务时向后台传送的数据
        submitAllForm() {
            ...
            if(this.options.length==0)
            {
                ...
                .catch(action => {
                    this.$router.push({path:'/facebook/HostConfig'})
                });
            }
            //先验证合法性
        ...
        WheatherToWorkStatus() {
            ...
            .catch(action => {
                this.$router.push({path:'/facebook/WorkStation'})
            });
        },
        WheatherToPeriodWorkStatus() {
            ...
            .catch(action => {
                this.$router.push({path:'/facebook/PeriodWorkStation'})
            });
        }
    }
}
